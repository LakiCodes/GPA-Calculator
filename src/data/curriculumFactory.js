export const normalizeId = (value) => value
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/^-|-$/g, "");
export const courseKey = (year, semester, pathwayId) => {
    const base = semester ? `Y${year}S${semester}` : `Y${year}`;
    return pathwayId ? `${base}:${pathwayId}` : base;
};
export const makeCourse = (programmeId, seed) => {
    const discriminator = [
        programmeId,
        seed.year,
        seed.semester,
...
export const makeElectiveGroup = (programmeId, seed) => {
    const availableCourses = seed.courses.map((course) => makeCourse(programmeId, {
        ...course,
        year: seed.year,
        semester: seed.semester,
        status: "elective",
...
const common = (programmeId, variant = "standard") => {
    ...
    return [...sem1, ...sem2].map(([code, title, credits, notionalHours]) => makeCourse(programmeId, {
        code: String(code),
        title: String(title),
