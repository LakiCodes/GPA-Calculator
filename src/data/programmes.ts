import type { Course, ElectiveGroup, Programme } from "../domain/types";
...
import {
  ...
  makeElectiveGroup,
  makeProgramme
} from "./curriculumFactory";

const make = (programmeId: string) => ({
  c: (seed: Parameters<typeof makeCourse>[1]) => makeCourse(programmeId, seed),
  g: (seed: Parameters<typeof makeElectiveGroup>[1]) =>
    makeElectiveGroup(programmeId, seed)
});

const accounting = (): Programme => {
...
const businessAdministration = (): Programme => {
...
const businessEconomics = (): Programme => {
  ...
  const commonRows = (year: 2 | 3 | 4, semester: 1 | 2, rows: string[], sourcePage: string): Course[] =>
    rows.map((row) => {
...
const bcom = (): Programme => {
  ...
  const rows = (year: 2 | 3 | 4, semester: 1 | 2, sourcePage: string, values: string[], pathwayIds?: string[]) =>
    values.map((row) => {
      const [code, title, credits = "3", hours = "150"] = row.split("|");
...
const operationsTechnology = (): Programme => {
...
const entrepreneurship = (): Programme => {
...
const realEstate = (): Programme => {
  ...
  const rows = (year: 2 | 3 | 4, semester: 1 | 2, sourcePage: string, values: string[]) =>
    values.map((row) => {
      const [code, title, credits = "3", hours = "150"] = row.split("|");
...
const finance = (): Programme => {
  ...
  const { c, g } = make(programmeId);
  const groupCourses = (sourceTable: string, entries: string[]) =>
    entries.map((row) => {
      const [code, title] = row.split("|");
...
const humanResourceManagement = (): Programme => {
...
const businessInformationSystems = (): Programme => {
...
const marketingManagement = (): Programme => {
...
const managementPublicPolicy = (): Programme => {
