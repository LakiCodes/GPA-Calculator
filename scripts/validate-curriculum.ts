...
import type { CurriculumSelection, ElectiveGroup, Programme } from "../src/domain/types";
...
import {
  ...
  getSelectionIssues,
  sumCredits
} from "../src/calculations/curriculum";
...
const parseKey = (
  key: string
): { year: 1 | 2 | 3 | 4; semester?: 1 | 2; pathwayId?: string } => {
  ...
  return {
    year: Number(match[1]) as 1 | 2 | 3 | 4,
    ...(match[2] ? { semester: Number(match[2]) as 1 | 2 } : {}),
    ...(pathwayId ? { pathwayId } : {})
  };
};

const selectCoursesForGroup = (group: ElectiveGroup, pathwayId?: string): string[] => {
  const available = group.availableCourses.filter(
    (course) => !course.pathwayIds?.length || Boolean(pathwayId && course.pathwayIds.includes(pathwayId))
  );
...
const defaultSelection = (
  ...
  electiveSelections: Object.fromEntries(
    getApplicableElectiveGroups(programme, {
      programmeId: programme.programmeId,
      ...(pathwayId ? { pathwayId } : {}),
      electiveSelections: {}
...
assert(
  programmes.length === expectedProgrammeNames.length,
  `Expected ${expectedProgrammeNames.length} programmes; found ${programmes.length}.`
);

expectedProgrammeNames.forEach((name, index) => {
  assert(
    programmes[index]?.programmeName === name,
    `Programme ${index + 1} must be "${name}"; found "${programmes[index]?.programmeName ?? "missing"}".`
  );
});
...
for (const programme of programmes) {
  ...
  assert(
    ...
    `${programme.programmeName}: duplicate course IDs detected.`
  );
  for (const group of programme.electiveGroups) {
    ...
    assert(
      group.minimumSelectionCount <= group.maximumSelectionCount,
      `${programme.programmeName}: ${group.title} has invalid elective min/max counts.`
    );
    assert(
      group.requiredSelectionCount >= group.minimumSelectionCount &&
        group.requiredSelectionCount <= group.maximumSelectionCount,
      `${programme.programmeName}: ${group.title} required count is outside allowed min/max.`
    );
  }
  ...
  for (const selection of selections) {
    ...
    assert(
      selectedCourses.every((course) => course.credits > 0 && course.notionalHours >= 0),
      `${label}: invalid course credit or notional-hour value.`
    );
    ...
    for (const key of Object.keys(programme.expectedTotals.semesterCredits)) {
      ...
      const actual = sumCredits(
        selectedCourses.filter(
          (course) => course.year === parsedKey.year && course.semester === parsedKey.semester
        )
      );
      assert(
        expectedMatches(expected, actual),
        `${label}: ${key} expected ${JSON.stringify(expected)} credits, calculated ${actual}.`
      );
    }
    ...
    for (const key of Object.keys(programme.expectedTotals.yearCredits)) {
      ...
      const actual = sumCredits(
        selectedCourses.filter((course) => course.year === parsedKey.year)
      );
      assert(
        expectedMatches(expected, actual),
        `${label}: ${key} expected ${JSON.stringify(expected)} credits, calculated ${actual}.`
    ...
    assert(
      expectedMatches(programme.expectedTotals.programmeCredits, programmeCredits),
      `${label}: programme total expected ${JSON.stringify(programme.expectedTotals.programmeCredits)}, calculated ${programmeCredits}.`
    );
    ...
    for (const year of [1, 2, 3, 4] as const) {
      for (const semester of [1, 2] as const) {
        ...
        if (expected === undefined) {
          ...
          assert(
            expectedMatches(expected, actual),
            `${label}: Y${year}S${semester} expected ${JSON.stringify(expected)}, calculated ${actual}.`
          );
        }
      }
      ...
      if (expected !== undefined) {
        assert(
          expectedMatches(expected, actual),
          `${label}: Y${year} expected ${JSON.stringify(expected)}, calculated ${actual}.`
        );
      }
    }
  }
